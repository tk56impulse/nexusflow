...
### 実施内容
- Tailwindが適用されない問題を修正
  - globals.css の読み込み確認
  - postcss.config.mjs を追加してビルド経路を修正
- ダッシュボード画面のUIを整備
  - ランキングとカレンダーの2カラムレイアウトを実装
  - カレンダーの表示領域を調整
- タスク追加機能の不具合修正
  - NEW TASKボタンのonClick処理を修正
- 開発環境の不整合を解消
  - キャッシュ削除・再ビルドで表示崩れを修正

### 課題・気づき
- Tailwindはインストールだけでは動かず、PostCSS設定が重要
- Next.jsのApp RouterではCSSのimportパスに注意が必要
- ビルドキャッシュ（.next）が原因で見た目が更新されないケースがある

### 次回やること
- UIの微調整（余白・ボタン統一）
- スクリーンショット取得（ポートフォリオ用）
- トップページの作成（導線強化）